...
export const useLinkWallet = () => {
  ...
  const safeConnectors = computed<AnyConnector[]>(() => {
    const maybeRef = (connectors as any)?.value
    if (Array.isArray(maybeRef)) {
      return maybeRef as AnyConnector[]
    }
    if (Array.isArray(connectors as any)) {
      return (connectors as any) as AnyConnector[]
    }
    return []
  })
  ...
  // navBar 的连接逻辑
  const wallconnects = async (walletId: string, targetChainId: number | bigint | string | undefined) => {
    const list = safeConnectors.value || []
    const connector = list.find((c: AnyConnector) => c.id === walletId)
    const finalConnector = connector || injected()
    await connect({ connector: finalConnector as any, chainId: targetChainId as any })
  }

  const handleConnect = async (wallet: WalletItem) => {
    try {
      isConnectingFromPage.value = true
      await wallconnects(wallet.id, chainId.value as any)
    } catch (error) {
  ...
  onMounted(() => {
    if (safeConnectors.value && safeConnectors.value.length > 0) {
      console.log(
        '✅ Connectors已就绪:',
        safeConnectors.value.map((c: any) => ({
          id: c.id,
          name: c.name,
          type: c.type
        }))
