          ...
          <div class="filter-container">
            ...
            <div class="tag-scroll-wrapper">
              ...
              >
                <!-- 当二级分类列表为空时，展示搜索框 -->
                <div
                  v-if="!ecosystemList || ecosystemList.length === 0"
                  class="esports-search"
                >
                  <svg
                    class="esports-search-icon"
                    viewBox="0 0 24 24"
                    fill="none"
                  ...
                  <input
                    class="esports-search-input"
                    type="text"
                    :placeholder="$t('home.searchPlaceholder') || '搜索'"
                    v-model="searchQuery"
              ...
              <!-- 右侧渐变遮罩 -->
              <div
                class="gradient-mask gradient-mask-right"
                v-if="ecosystemList && ecosystemList.length > 0"
              ></div>
            </div>
            ...
            <div class="action-buttons">
              ...
              <div
                ...
                :class="{ active: activeTag === 'favorite' }"
            ...
            <div v-if="showFilterPanel" class="filter-panel">
              <div class="filter-dropdowns-row">
                ...
                <div class="filter-select-wrapper">
                  <button
                    class="filter-select-btn"
                    @click.stop="toggleFrequencyDropdown"
                  >
                    <div class="filter-label">
                    ...
                    <el-icon
                      class="filter-arrow"
                      :class="{ rotate: showFrequencyList }"
                    >
                      <ArrowDownBold />
                    </el-icon>
                  </button>
          ...
          <div v-else class="card-list">
            <template v-for="(item, index) in cardList" :key="'card-' + index">
              ...
              <div v-if="isNewUserEventsCategory" class="new-user-list-item">
                ...
                <div class="new-user-actions">
                  <button
                    class="new-user-action-btn"
                    @click="
                      navigateToDetail(
                        item,
                        item.options[0]?.directions[1]?.outcome,
                      )
                    "
                  >
                    {{
                      getDisplayOutcome(item.options[0]?.directions[1]?.outcome)
                    }}
                  </button>
                  <button
                    class="new-user-action-btn"
                    @click="
                      navigateToDetail(
                        item,
                        item.options[0]?.directions[0]?.outcome,
                      )
                    "
                  >
                    {{
                      getDisplayOutcome(item.options[0]?.directions[0]?.outcome)
                    }}
                  </button>
                </div>
              ...
              >
                ...
                <div class="item-amount">
                  <div class="amount-left">
                    <div
                      class="time-info"
                      :class="{ urgent: item.isTimeUrgent }"
                    >
                      <span
              ...
              <div v-else class="list-item small-item">
                ...
                <div
                  class="item-options"
                  :class="{ 'scrollable-options': item.options.length > 3 }"
                    ...
                    <div class="option-buttons">
                      <button
                        ...
                        @click.stop="
                          navigateToDetail(
                            item,
                            opt?.directions[1]?.outcome,
                            opt.subEventGuid,
                          )
                        "
                      >
                        {{ getDisplayOutcome(opt?.directions[1]?.outcome) }}
                      </button>
                      <button
                        ...
                        @click.stop="
                          navigateToDetail(
                            item,
                            opt?.directions[0]?.outcome,
                            opt.subEventGuid,
                          )
                        "
                      >
                        {{ getDisplayOutcome(opt?.directions[0]?.outcome) }}
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <div class="load-more-footer">
              <span v-if="loadingMore" class="load-more-text">{{
                $t("home.loadingMore") || "加载中..."
              }}</span>
              <span
                v-else-if="cardList.length && !hasMore"
                class="load-more-text"
                >{{ $t("home.noMore") || "没有更多了" }}</span
              >
            </div>
          </div>
        </div>
...
const currentSortLabel = computed(
  () =>
    sortOptions.value.find((opt) => opt.value === selectedSort.value)?.label ||
    t("home.sortAll") ||
    "全部",
);
const currentFrequencyLabel = computed(
  () =>
    frequencyOptions.value.find((opt) => opt.value === selectedFrequency.value)
      ?.label || "全部频率",
);
...
const fetchEventList = async (append = false) => {
  ...
  try {
    ...
    if (
      ...
      params.ecosystem_guid = activeTag.value;
    if (searchQuery.value) params.title = searchQuery.value;
    if (selectedFrequency.value && selectedFrequency.value !== "all")
      params.event_period_guid = selectedFrequency.value;
    if (selectedSort.value && selectedSort.value !== "all")
...
const navigateToDetail = (item, choice, subEventGuid) => {
  ...
  const isProd =
    import.meta.env.VITE_API_ADDRESS === "https://api.roothashpay.com";
  // 部分列表可能你配置的 GUID 实际落在 sub_event_guid 上
  if (!item?.title.includes("BTCUSDT") && isProd) {
  ...
  if (item?.options?.length === 1) {
    const cryptoQuery = {
      ...
      ...(isNewUserEventsCategory.value
        ? { from_new_user_compensation: "1" }
        : {}),
    };
    router.push({
