  ...
  <div class="position-list">
    ...
    <div class="position-tabs">
      <div
        v-for="tab in tabs"
        :key="tab.value"
        :class="['position-tab-item', { active: activeTab === tab.value }]"
      ...
      >
        {{
          cancelAllLoading
            ...
            : t("assetManagement.cancelAllOrders") || "Cancel all"
        }}
      </button>
        ...
        <div class="position-item">
          ...
          <div class="position-middle">
            ...
            <div class="meta-row">
              <span
                class="side-tag"
                :class="String(item.side || '').toLowerCase()"
              >
                {{
                  item.side === "SELL"
                    ? t("assetManagement.sell")
                    : t("assetManagement.buy")
                }}
              </span>
              ...
              <template v-if="activeTab === 'open'">
                <span
                  class="pnl-text"
                  :class="item.pnl > 0 ? 'pnl-positive' : 'pnl-negative'"
                >
                  {{ item.pnl > 0 ? "+" : "" }}{{ item.pnl }}%
                </span>
              </template>
            ...
            <div class="sub-row">
              <span class="value-text"
                >{{ t("assetManagement.valueLabel") }}: ${{ item.value }}</span
              >
        ...
        <div class="custom-bottom-modal">
          ...
          <div class="modal-actions">
            <PrimaryActionButton
              ...
              font-size="16px"
              font-weight="600"
              text-color="var(--bg-opposite, #fff)"
              gradient-from="var(--bg-pn, #2a2a2a)"
              gradient-to="var(--bg-pn, #2a2a2a)"
              @click="onCancelClose"
            >
              {{ $t("common.cancel") || "取消" }}
            </PrimaryActionButton>
            <PrimaryActionButton
              ...
              font-weight="600"
              text-color="#000000"
              gradient-from="var(--text-color-y, #ffd94b)"
              gradient-to="var(--text-color-y, #ffd94b)"
...
const currentTypeLabel = computed(() => {
  return (
    typeOptions.value.find((v) => v.value === filterType.value)?.label ??
    t("assetManagement.all")
  );
});

const currentMonthLabel = computed(() => {
  return (
    monthOptions.value.find((v) => v.value === filterMonth.value)?.label ??
    (t("common.all") || "All")
  );
});
  ...
  try {
    ...
    ElMessage.success(
      res?.data?.data?.message ||
        res?.data?.message ||
        t("assetManagement.cancelSuccess") ||
        "Canceled",
    );
    resetAndFetch();
  } catch (e) {
...
const getSideFilterParam = () => {
  ...
  if (filterType.value === "BUY" || filterType.value === "SELL")
...
const mapPositionToRow = (p) => {
  ...
  const pnlPct =
    Number.isFinite(bet) && bet !== 0 && Number.isFinite(pnlAbs)
      ? (pnlAbs / bet) * 100
      : Number(p?.lost_rate);

  const positionValue = Number(p?.position_value);
  const time = (p?.trade_at || p?.updated_at || p?.created_at || "")
    .replace("T", " ")
    .replace("Z", "");
...
const mapOpenOrderToRow = (o) => {
  ...
  const progressText =
    Number.isFinite(dealedSizeNum) && Number.isFinite(sizeNum) && sizeNum > 0
  ...
  const time = (o?.expire_at || o?.updated_at || o?.created_at || "")
    .replace("T", " ")
    .replace("Z", "");

  const costNum = Number(o?.cost);
  const valueText = Number.isFinite(costNum)
    ? costNum.toFixed(2)
    : o?.cost || "0.00";
    ...
    const rows =
      activeTab.value === "pending"
        ...
        : activeTab.value === "history"
          ? (Array.isArray(data.orders) ? data.orders : []).map(
              mapOrderHistoryToRow,
            )
...
onMounted(() => {
  ...
  if (!filterMonth.value)
    filterMonth.value = monthOptions.value[0]?.value || "";
  resetAndFetch();
});
