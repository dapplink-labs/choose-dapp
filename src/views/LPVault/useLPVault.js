...
export const useLPVault = () => {
  ...
  const activatedNodeType = ref("");
  const activatedNodeName = ref("");
  ...
  const fetchNodeStakingList = async () => {
    ...
    try {
      ...
      // 将接口数据映射到组件需要的格式
      nodeListData.value = list
        .sort(
          (a, b) =>
            parseInt(a.node_level.match(/\d+/)[0]) -
            parseInt(b.node_level.match(/\d+/)[0]),
        )
        .map((item) => {
          return {
            id: item.id,
            ...
            price: String(
              formatUnits(BigInt(item.staking_amount.toString()), 18),
            ),
            dailyEarnings: String(item.node_income || 0) + "%",
            cycleDays: item.node_period
              ? `${item.node_period}${t("lpVault.days")}`
              : `0${t("lpVault.days")}`,
            totalEarnings: String(
              formatUnits(BigInt(item.forecast_income.toString()), 18) || "0",
            ),
  ...
  const handleActivate = async (type) => {
    ...
    try {
      ...
      const { proxyStakingManager, usdtTokenAddress, proxyNodeManager } =
        bscNet;
      ...
      // 显示成功弹窗
      activatedNodeType.value = nodeItem.nodeLevel;
      activatedNodeName.value = nodeItem.name;
